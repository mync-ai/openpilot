...
SeatControlConfigDialog::SeatControlConfigDialog(QWidget *parent)
    ...
    setFixedSize(800, 750); // Increase height to accommodate larger spacing
    setModal(true);
...
void SeatControlConfigDialog::setupUI() {
    ...
    description_label->setStyleSheet("font-size: 16px; color: #cccccc; margin-bottom: 10px;");
    ...
    params_group->setStyleSheet(
        "QGroupBox { font-size: 18px; font-weight: bold; color: #ffffff; "
        "border: 2px solid #666666; border-radius: 10px; margin-top: 10px; padding-top: 10px; }"
        "QGroupBox::title { subcontrol-origin: margin; left: 20px; }"
    );

    QGridLayout *params_layout = new QGridLayout(params_group);
    params_layout->setContentsMargins(20, 30, 20, 20);
    params_layout->setVerticalSpacing(18); // Moderate vertical spacing between rows
    ...
    auto create_label = [](const QString &text, const QString &description = "") {
        ...
        layout->setSpacing(4); // Space between label and description

        QLabel *label = new QLabel(text);
        label->setStyleSheet("font-size: 16px; font-weight: bold; color: #ffffff;");
        label->setMinimumHeight(20); // Ensure label has proper height
        ...
        if (!description.isEmpty()) {
            ...
            desc->setMinimumHeight(16); // Ensure description has proper height
        ...
        container->setMinimumHeight(description.isEmpty() ? 25 : 45); // Ensure container has enough height
    ...
    frequency_spin->setStyleSheet("font-size: 14px; padding: 8px; min-height: 35px;");
    frequency_spin->setMinimumHeight(35);
    ...
    turn_thresh_1_spin->setStyleSheet("font-size: 14px; padding: 8px; min-height: 35px;");
    turn_thresh_1_spin->setMinimumHeight(35);
    ...
    turn_thresh_2_spin->setStyleSheet("font-size: 14px; padding: 8px; min-height: 35px;");
    turn_thresh_2_spin->setMinimumHeight(35);
    ...
    long_thresh_spin->setStyleSheet("font-size: 14px; padding: 8px; min-height: 35px;");
    long_thresh_spin->setMinimumHeight(35);
    ...
    smoothing_window_spin->setStyleSheet("font-size: 14px; padding: 8px; min-height: 35px;");
    smoothing_window_spin->setMinimumHeight(35);
    ...
    horizon_spin->setStyleSheet("font-size: 14px; padding: 8px; min-height: 35px;");
    horizon_spin->setMinimumHeight(35);
    ...
    use_plan_check->setStyleSheet("font-size: 14px; min-height: 35px;");
    use_plan_check->setMinimumHeight(35);
    ...
    validation_label->setStyleSheet("font-size: 14px; color: #ff6666; font-weight: bold;");
    ...
    load_button->setStyleSheet(
        "QPushButton { background-color: #4a90e2; color: white; font-size: 16px; "
    ...
    status_label->setStyleSheet("font-size: 14px; color: #66cc66;");
    ...
    progress_bar->setStyleSheet(
        "QProgressBar { border: 2px solid #666666; border-radius: 5px; text-align: center; }"
    ...
    button_box->button(QDialogButtonBox::Save)->setStyleSheet(
        ...
        "padding: 10px 25px; border-radius: 8px; font-weight: bold; }"
        "QPushButton:hover { background-color: #229954; }"
        "QPushButton:pressed { background-color: #1e8449; }"
        "QPushButton:disabled { background-color: #7f8c8d; }"
    );
    button_box->button(QDialogButtonBox::Cancel)->setStyleSheet(
        "QPushButton { background-color: #95a5a6; color: white; font-size: 16px; "
        "padding: 10px 25px; border-radius: 8px; font-weight: bold; }"
    ...
    setStyleSheet(
        ...
        "  background-color: #34495e; color: #ffffff; border: 1px solid #666666; "
        "  border-radius: 5px; padding: 5px; font-size: 14px; }"
        "QSpinBox::up-button, QDoubleSpinBox::up-button { "
        "  background-color: #4a90e2; border: 1px solid #357abd; "
        "  border-top-right-radius: 5px; width: 20px; color: #ffffff; "
        "  font-size: 14px; font-weight: bold; }"
        "QSpinBox::up-button:hover, QDoubleSpinBox::up-button:hover { "
        "  background-color: #357abd; }"
        "QSpinBox::down-button, QDoubleSpinBox::down-button { "
        "  background-color: #4a90e2; border: 1px solid #357abd; "
        "  border-bottom-right-radius: 5px; width: 20px; color: #ffffff; "
        "  font-size: 14px; font-weight: bold; }"
        "QSpinBox::down-button:hover, QDoubleSpinBox::down-button:hover { "
        "  background-color: #357abd; }"
        "QCheckBox { color: #ffffff; font-size: 14px; }"
        "QCheckBox::indicator { width: 18px; height: 18px; }"
        "QCheckBox::indicator:unchecked { background-color: #34495e; border: 1px solid #666666; }"
